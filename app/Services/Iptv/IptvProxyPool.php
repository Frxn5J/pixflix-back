...
class IptvProxyPool
{
    ...
    /**
     * Returns the URL that should actually be fetched for $target, routed
     * through the next proxy in the rotation without performing the request.
     * Used by X-Accel delivery: each client retry lands on the next proxy,
     * so failover keeps working without holding a PHP worker.
     */
    public function wrap(string $target): string
    {
        $proxies = $this->configured();
        if ($proxies === []) {
            return $target;
        ...
        $proxy = $proxies[$this->nextStart(count($proxies))];

        return $this->proxyUrl($proxy['base_url'], $target);
    }
