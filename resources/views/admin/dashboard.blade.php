...
@if ($section === 'overview')
    ...
    <section class="panel" style="margin-top:18px">
        ...
        <p class="muted">Un único addon Stremio abastece el catálogo y la reproducción de VOD. No hay fuentes alternativas.</p>
        <div class="actions"><a class="button primary" href="{{ route('admin.dashboard', ['section' => 'fallback']) }}">Administrar VOD</a></div>
    </section>
...
@elseif ($section === 'stremio-catalog')
    @php($catalog = $streamFallback['catalog'] ?? [])
    <section class="panel"><div class="panel-heading"><div><p class="eyebrow">FUENTE DE CATÁLOGO</p><h2>Stremio catálogo</h2><p class="muted">Estos addons son los únicos que alimentan el inventario de películas y series de Pixflix.</p></div><span class="status @if (!($catalog['enabled'] ?? false)) off @endif">{{ ($catalog['enabled'] ?? false) ? 'Activo' : 'Inactivo' }}</span></div>
    <form method="POST" action="{{ route('admin.stream-fallback.update') }}">@csrf @method('PUT')<input type="hidden" name="stremio_role" value="catalog"><div class="grid-2"><label class="checkbox"><input type="hidden" name="enabled" value="0"><input type="checkbox" name="enabled" value="1" @checked($catalog['enabled'] ?? false)> Activar catálogo Stremio</label><div class="field"><label>Timeout (segundos)</label><input name="timeout_seconds" type="number" min="1" max="60" value="{{ $catalog['timeout_seconds'] ?? 10 }}" required></div></div><h3 style="margin-top:24px">Addons de catálogo</h3>@forelse ($catalog['addons'] ?? [] as $index => $addon)<div class="source-row"><input type="hidden" name="addons[{{ $index }}][id]" value="{{ $addon['id'] }}"><div class="field"><label>Nombre</label><input name="addons[{{ $index }}][name]" value="{{ $addon['name'] }}" required></div><div class="field wide"><label>URL de instancia o manifest.json</label><input type="url" name="addons[{{ $index }}][base_url]" value="{{ $addon['base_url'] }}" required></div><div class="field"><label>Prioridad</label><input name="addons[{{ $index }}][priority]" type="number" min="1" value="{{ $addon['priority'] }}" required></div><div class="field"><label>Timeout</label><input name="addons[{{ $index }}][timeout_seconds]" type="number" min="1" max="60" value="{{ $addon['timeout_seconds'] ?? ($catalog['timeout_seconds'] ?? 10) }}" required></div><label class="checkbox"><input type="hidden" name="addons[{{ $index }}][enabled]" value="0"><input type="checkbox" name="addons[{{ $index }}][enabled]" value="1" @checked($addon['enabled'] ?? true)> Activo</label><button class="button small danger" type="submit" formaction="{{ route('admin.stream-fallback.addon.remove', $addon['id']) }}" formmethod="POST" name="_method" value="DELETE" onclick="return confirm('¿Eliminar este addon?')">Eliminar</button></div>@empty<p class="empty">No hay addons de catálogo configurados.</p>@endforelse<div class="actions"><button class="button primary" type="submit">Guardar catálogo</button></div></form>
    <form method="POST" action="{{ route('admin.stream-fallback.sync-catalog') }}" class="actions" style="margin-top:12px">@csrf<button class="button" type="submit">Importar catálogo ahora</button></form></section>
    @if (session('verification'))<div class="result"><strong>Resultado de verificación del addon de catálogo</strong><pre>{{ json_encode(session('verification'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre></div>@endif
    <section class="panel"><h3>Agregar addon de catálogo</h3><p class="hint">Usa la URL de una instancia instalada o de su manifest. La página de Stremio Addons solo sirve para localizar el addon.</p><form method="POST" action="{{ route('admin.stream-fallback.addon') }}">@csrf<input type="hidden" name="stremio_role" value="catalog"><div class="form-grid"><div class="field"><label>Nombre</label><input name="name" value="{{ old('name') }}" required></div><div class="field wide"><label>URL de instancia o manifest.json</label><input name="base_url" type="url" value="{{ old('base_url') }}" placeholder="https://addon.example/manifest.json" required></div><div class="field"><label>Prioridad</label><input name="priority" type="number" min="1" value="{{ old('priority', 1) }}" required></div><div class="field"><label>Timeout</label><input name="timeout_seconds" type="number" min="1" max="60" value="{{ old('timeout_seconds', $catalog['timeout_seconds'] ?? 10) }}" required></div></div><div class="actions"><button class="button primary" type="submit">Verificar e instalar addon</button></div></form></section>
@elseif ($section === 'stremio-streams')
    @php($streams = $streamFallback['streams'] ?? [])
    <section class="panel"><div class="panel-heading"><div><p class="eyebrow">FUENTE DE REPRODUCCIÓN</p><h2>Stremio reproducción</h2><p class="muted">Estos addons solo se consultan para encontrar streams. Sus catálogos no se importan ni se contabilizan.</p></div><span class="status @if (!($streams['enabled'] ?? false)) off @endif">{{ ($streams['enabled'] ?? false) ? 'Activo' : 'Inactivo' }}</span></div>
    <form method="POST" action="{{ route('admin.stream-fallback.update') }}">@csrf @method('PUT')<input type="hidden" name="stremio_role" value="streams"><div class="grid-2"><label class="checkbox"><input type="hidden" name="enabled" value="0"><input type="checkbox" name="enabled" value="1" @checked($streams['enabled'] ?? false)> Activar reproducción Stremio</label><label class="checkbox"><input type="hidden" name="primary" value="0"><input type="checkbox" name="primary" value="1" @checked($streams['primary'] ?? false)> Usar Stremio como fuente principal</label></div><div class="form-grid" style="margin-top:15px"><div class="field"><label>Idiomas permitidos, separados por coma</label><input name="languages_csv" value="{{ implode(', ', $streams['languages'] ?? []) }}" placeholder="Latino, Español, English"></div><div class="field"><label>Timeout general (segundos)</label><input name="timeout_seconds" type="number" min="1" max="60" value="{{ $streams['timeout_seconds'] ?? 10 }}" required></div><div class="field"><label>Cache de resoluciones (segundos)</label><input name="cache_ttl_seconds" type="number" min="60" max="604800" value="{{ $streams['cache_ttl_seconds'] ?? 1800 }}" required></div></div><h3 style="margin-top:24px">Addons de reproducción</h3>@forelse ($streams['addons'] ?? [] as $index => $addon)<div class="source-row"><input type="hidden" name="addons[{{ $index }}][id]" value="{{ $addon['id'] }}"><div class="field"><label>Nombre</label><input name="addons[{{ $index }}][name]" value="{{ $addon['name'] }}" required></div><div class="field wide"><label>URL de instancia o manifest.json</label><input type="url" name="addons[{{ $index }}][base_url]" value="{{ $addon['base_url'] }}" required></div><div class="field"><label>Prioridad</label><input name="addons[{{ $index }}][priority]" type="number" min="1" value="{{ $addon['priority'] }}" required></div><div class="field"><label>Timeout</label><input name="addons[{{ $index }}][timeout_seconds]" type="number" min="1" max="60" value="{{ $addon['timeout_seconds'] ?? ($streams['timeout_seconds'] ?? 10) }}" required></div><label class="checkbox"><input type="hidden" name="addons[{{ $index }}][enabled]" value="0"><input type="checkbox" name="addons[{{ $index }}][enabled]" value="1" @checked($addon['enabled'] ?? true)> Activo</label><button class="button small danger" type="submit" formaction="{{ route('admin.stream-fallback.addon.remove', $addon['id']) }}" formmethod="POST" name="_method" value="DELETE" onclick="return confirm('¿Eliminar este addon?')">Eliminar</button></div>@empty<p class="empty">No hay addons de reproducción configurados.</p>@endforelse<div class="actions"><button class="button primary" type="submit">Guardar reproducción</button></div></form></section>
    @if (session('verification'))<div class="result"><strong>Resultado de verificación del addon de reproducción</strong><pre>{{ json_encode(session('verification'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre></div>@endif
    <section class="panel"><h3>Agregar addon de reproducción</h3><p class="hint">Puedes agregar addons de distintas procedencias. Solo deben publicar el recurso <code>stream</code>.</p><form method="POST" action="{{ route('admin.stream-fallback.addon') }}">@csrf<input type="hidden" name="stremio_role" value="streams"><div class="form-grid"><div class="field"><label>Nombre</label><input name="name" value="{{ old('name') }}" required></div><div class="field wide"><label>URL de instancia o manifest.json</label><input name="base_url" type="url" value="{{ old('base_url') }}" placeholder="https://addon.example/manifest.json" required></div><div class="field"><label>Prioridad</label><input name="priority" type="number" min="1" value="{{ old('priority', 1) }}" required></div><div class="field"><label>Timeout</label><input name="timeout_seconds" type="number" min="1" max="60" value="{{ old('timeout_seconds', $streams['timeout_seconds'] ?? 10) }}" required></div></div><div class="actions"><button class="button primary" type="submit">Verificar e instalar addon</button></div></form></section>
@elseif ($section === 'fallback')
    <section class="panel"><div class="panel-heading"><div><p class="eyebrow">FUENTE VOD ÚNICA</p><h2>Addon Stremio</h2><p class="muted">Este mismo addon provee el catálogo y los streams de películas y series.</p></div><span class="status @if (!($streamFallback['enabled'] ?? false)) off @endif">{{ ($streamFallback['enabled'] ?? false) ? 'Activo' : 'Inactivo' }}</span></div>
    <form method="POST" action="{{ route('admin.stream-fallback.update') }}">@csrf @method('PUT')<div class="grid-2"><label class="checkbox"><input type="hidden" name="enabled" value="0"><input type="checkbox" name="enabled" value="1" @checked($streamFallback['enabled'] ?? false)> Activar VOD Stremio</label></div><div class="form-grid" style="margin-top:15px"><div class="field"><label>Idiomas permitidos, separados por coma</label><input name="languages_csv" value="{{ implode(', ', $streamFallback['languages'] ?? []) }}" placeholder="Latino, Español, English"></div><div class="field"><label>Timeout general (segundos)</label><input name="timeout_seconds" type="number" min="1" max="60" value="{{ $streamFallback['timeout_seconds'] ?? 10 }}" required></div><div class="field"><label>Cache de resoluciones (segundos)</label><input name="cache_ttl_seconds" type="number" min="60" max="604800" value="{{ $streamFallback['cache_ttl_seconds'] ?? 1800 }}" required></div></div><h3 style="margin-top:24px">Addon configurado</h3>@forelse ($streamFallback['addons'] ?? [] as $index => $addon)<div class="source-row"><input type="hidden" name="addons[{{ $index }}][id]" value="{{ $addon['id'] }}"><div class="field"><label>Nombre</label><input name="addons[{{ $index }}][name]" value="{{ $addon['name'] }}" required></div><div class="field wide"><label>URL base o manifest.json</label><input type="url" name="addons[{{ $index }}][base_url]" value="{{ $addon['base_url'] }}" required></div><input type="hidden" name="addons[{{ $index }}][priority]" value="1"><div class="field"><label>Timeout</label><input name="addons[{{ $index }}][timeout_seconds]" type="number" min="1" max="60" value="{{ $addon['timeout_seconds'] ?? ($streamFallback['timeout_seconds'] ?? 10) }}"></div><label class="checkbox"><input type="hidden" name="addons[{{ $index }}][enabled]" value="0"><input type="checkbox" name="addons[{{ $index }}][enabled]" value="1" @checked($addon['enabled'])> Activo</label></div>@empty<p class="empty">No hay addon VOD configurado.</p>@endforelse<div class="actions"><button class="button primary" type="submit">Guardar addon VOD</button><button class="button" type="submit" formaction="{{ route('admin.stream-fallback.sync-catalog') }}" formmethod="POST" name="_method" value="POST">Importar catálogo</button></div></form></section>
    <section class="panel">
